...
def smart_process_text_input(text_content: str, primary_lang: str, session_path: Path) -> Dict:
    ...
    if best_match[2] and best_match[1] >= 0.4:
        ...
        if file_type == 'python':
            logger.info(f"Detected Python content (confidence: {confidence})")
            ...
            logger.info(f"Detected SQL content (confidence: {confidence})")
        ...
        logger.info(f"Detected HTML content (confidence: {html_confidence})")
...
def run_pipeline_async(
    ...
    log.info("Starting pipeline for session %s", session_id)
    ...
    with app.app_context():
        ...
        try:
            if hasattr(app, "session_manager") and app.session_manager:
                ...
                session_path = app.session_manager.get_session_path(session_id)
                log.info("Created new session manager, path: %s", session_path)
            ...
            log.warning("Using project fallback session path: %s", session_path)

        # Log the final path for debugging
        log.info("Final session path: %s", session_path)
        ...
        try:
            ...
            log.exception("Pipeline failed for %s", session_id)
            tracker.fail(session_id, "pipeline_error", str(exc))
# ───────────────────────────── UI pages ───────────────────────────────────────
# ───────────────────────────── LOGIN ROUTES ────────────────────────────────────
...
def results(session_id: str) -> str:
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.warning(f"Could not check session metadata for {session_id}: {e}")
...
def textresults(session_id: str) -> str:
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.error(f"Error loading text results for {session_id}: {e}")
...
def metrics(session_id: str) -> str:
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.error(f"Error loading metrics data for session {session_id}: {e}")
...
def download_consolidated_metrics(session_id: str):
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.error(f"Error downloading consolidated metrics for {session_id}: {e}")
...
def download_memory_batch(session_id: str):
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.error(f"Error downloading memory batch for {session_id}: {e}")
            return jsonify(error="Download failed"), 500
...
def language_comparison(session_id: str, filename: str) -> str:
    ...
    if hasattr(current_app, "session_manager"):
        try:
            ...
            logger.error(f"JSON decode error for {session_id}/{filename}: {e}")
            error_message = f"Error reading JSON data: Invalid format"
        except Exception as e:
            logger.error(f"Error loading translation data for {session_id}/{filename}: {e}")
...
def api_progress(session_id: str):
    try:
        ...
        logger.exception("Progress polling error for %s", session_id)
...
def api_progress_stream(session_id: str):
    ...
    def generate():
        try:
            ...
            logger.info("Client closed SSE for %s", session_id)
        except Exception as exc:
...
def api_regenerate(session_id: str):
    ...
    try:
        ...
        logger.info(f"[Regenerate] Processing {translation_type} for session {session_id}")
        ...
        logger.info(f"[Regenerate] Using file: {basename}")
    ...
    # Clean the text content - remove any block identifiers that might be in the text
            ...
            if len(clean_text) > MAX_TEXT_LENGTH:
                logger.warning(f"[Regenerate] Text for {block_key} truncated from {len(clean_text)} to {MAX_TEXT_LENGTH} characters")
            ...
            if clean_text:
                ...
                logger.info(f"[Regenerate] Added clean segment {block_key}: {clean_text[:50]}...")
        ...
        logger.info(f"[Regenerate] Looking for script at: {script_path}")
        
        if not script_path.exists():
            logger.error(f"[Regenerate] Script not found: step4_merge.py at {script_path}")
            ...
            if result.returncode == 0:
                ...
                for pattern in actual_output_patterns:
                    ...
                    if potential_file.exists():
                        ...
                        logger.info(f"[Regenerate] Found generated file: {actual_output_file}")
                        break
                
                # If specific patterns don't work, search for any recently created HTML files
                if not actual_output_file:
                    logger.info(f"[Regenerate] Searching for any recent HTML files in {output_dir}")
                    for html_file in output_dir.glob("*.html"):
                        ...
                        if time.time() - html_file.stat().st_mtime < 60:
                            ...
                            logger.info(f"[Regenerate] Found recent HTML file: {actual_output_file}")
                            break
                
                if actual_output_file and actual_output_file.exists():
                    logger.info(f"[Regenerate] Serving file: {actual_output_file} (size: {actual_output_file.stat().st_size} bytes)")
                    ...
                    logger.error(f"[Regenerate] No output file found in {output_dir}")
                    # List all files for debugging
                    all_files = list(output_dir.glob("*"))
                    logger.error(f"[Regenerate] Files in output directory: {[f.name for f in all_files]}")
...
def _send_zip(session_id: str, filename: str):
    if hasattr(current_app, "session_manager"):
        ...
        logger.info(f"[DEBUG] Session path: {spath}")
...
def api_metrics(session_id: str):
    ...
    try:
        ...
        logger.exception("Metrics API error for %s", session_id)
        return jsonify(error=str(exc)), 500
...
@bp.route("/debug/find-zips/<session_id>")
def debug_find_zips(session_id: str):
    """Find where ZIP files are actually being stored"""
    if not session.get("authenticated"):
        return redirect(url_for("main.login"))
    ...
    # Search common locations for ZIP files with this session ID
    search_paths = [
        "/home/runner/workspace/",
        "/home/runner/workspace/sessions/",
        f"/home/runner/workspace/sessions/{session_id}/",
        "/tmp/",
        f"/tmp/{session_id}/",
        f"/tmp/sessions/{session_id}/",
        os.getcwd(),
        f"{os.getcwd()}/sessions/",
        f"{os.getcwd()}/sessions/{session_id}/",
    ]
    ...
    for search_path in search_paths:
        try:
            ...
            if path_obj.exists():
                # Find ZIP files
                zip_files = list(path_obj.rglob("*.zip"))
                # Filter for files containing session ID or recent files
                relevant_zips = [
                    str(zf) for zf in zip_files 
                    if session_id in str(zf) or 
                       (zf.stat().st_mtime > (time.time() - 3600))  # Modified in last hour
                ]
                
                if relevant_zips:
                    results["search_results"][search_path] = {
                        "path_exists": True,
                        "zip_files": relevant_zips
                    }
                else:
                    results["search_results"][search_path] = {
                        "path_exists": True,
    ...
    # Also run a system-wide find command
    try:
        import subprocess
        # Search for ZIP files containing the session ID
        find_result = subprocess.run(
            ["find", "/home/runner/workspace", "-name", "*.zip", "-type", "f"],
            capture_output=True,
            text=True,
            timeout=10
        )
        
        all_zips = find_result.stdout.strip().split('\n') if find_result.stdout.strip() else []
        session_zips = [zf for zf in all_zips if session_id in zf]
        
        results["system_find"] = {
            "all_zip_count": len(all_zips),
            "session_related_zips": session_zips,
            "recent_zips": []
        }
            ...
            try:
                if os.path.exists(zip_file):
                    ...
                    if time.time() - mtime < 3600:  # Modified in last hour
                        results["system_find"]["recent_zips"].append({
                            "file": zip_file,
                            "modified_ago_minutes": int((time.time() - mtime) / 60)
                        })
            except:
                pass
                
    except Exception as e:
        results["system_find"] = {"error": str(e)}
